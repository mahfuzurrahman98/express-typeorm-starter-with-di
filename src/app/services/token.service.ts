...
export class TokenService {
    ...
    createAccessToken(user: RequestUser): string {
        ...
        try {
            ...
            const expiryMinutes = parseInt(process.env.ACCESS_TOKEN_EXPIRY as string);
            expiresIn = expiryMinutes * 60 || 60 * 5; // Default to 5 minutes
        } catch (error: any) {
            throw error instanceof CustomError
                ? error
                : new CustomError(500, `[tokenService_createAccessToken_expiry]: ${error.message}`);
        }
        ...
        try {
            ...
            throw error instanceof CustomError
                ...
                : new CustomError(
                      401,
                      `[tokenService_createAccessToken_sign]: ${error.message || 'Unauthorized'}`,
                  );
        }
    }
    ...
    createRefreshToken(user: RequestUser): string {
        ...
        try {
            ...
            const expiryDays = parseInt(process.env.REFRESH_TOKEN_EXPIRY as string);
        ...
        try {
            ...
            throw error instanceof CustomError
                ...
                : new CustomError(
                      401,
                      `[tokenService_createRefreshToken_sign]: ${error.message || 'Unauthorized'}`,
                  );
        }
    }
    ...
    decodeAccessToken(token: string): TokenPayload {
        try {
            ...
            throw error instanceof CustomError
                ...
                : new CustomError(
                      401,
                      `[tokenService_decodeAccessToken]: ${error.message || 'Unauthorized'}`,
                  );
        }
    }
    ...
    decodeRefreshToken(token: string): TokenPayload {
        try {
            ...
            throw error instanceof CustomError
                ...
                : new CustomError(
                      401,
                      `[tokenService_decodeRefreshToken]: ${error.message || 'Unauthorized'}`,
                  );
        }
    }
}
